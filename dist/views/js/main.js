...
var resizePizzas = function(size) {
  ...
  function changeSliderLabel(size) {
    switch(size) {
      case "1":
        document.getElementById("pizzaSize").innerHTML = "Small";	//changed each case from querySelector to efficient getElementById
        return;
      case "2":
  ...
  var pizzaNewSize = document.getElementsByClassName("randomPizzaContainer");
  var pizzaNewLength = pizzaNewSize.length; 
 
  /*
  Made the following function more efficient
  Iterates through pizza elements on the page and changes their widths
  */
  function changePizzaSizes(size) {
	var newWidth; 			//new variable used to update pizza slider values

    switch(size) { 			//checks each case for the slider and returns the assigned newWidth value
      case "1":
        newWidth = 25;
        break;
      case "2":
	...
	//loops over randomPizzaContainer and updates its style.width to the corresponding newWidth percentage
    for (var i = 0; i < pizzaNewLength; i++) { 
    pizzaNewSize[i].style.width = newWidth + '%';
	}
  }
